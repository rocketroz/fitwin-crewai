...
except ImportError:  # pragma: no cover - fallback when CrewAI is unavailable
    def tool(name: str):  # type: ignore
        """Lightweight decorator shim used during tests without CrewAI."""
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            response = requests.post(
                url, json=measurement_data, headers=headers, timeout=TIMEOUT
            )
            ...
            if response.status_code == 422:
                ...
                return {"error": error_data.get("detail", {}), "status_code": 422}
...
def recommend_sizes(normalized_measurements: dict) -> dict:
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            response = requests.post(
                url, json=normalized_measurements, headers=headers, timeout=TIMEOUT
            )
